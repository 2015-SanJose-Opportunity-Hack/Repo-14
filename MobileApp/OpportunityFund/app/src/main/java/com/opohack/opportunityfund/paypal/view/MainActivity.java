        ...
        switch (v.getId()) {
            case R.id.btn_signup:
                ...
                try {
                    ...
                    jsonObject.put("downpayment_loan", mDownPayment.getText().toString().trim());
                    jsonObject.put("loan_fee", mLoanFee.getText().toString().trim());
                    jsonObject.put("interest_rate", mRate.getText().toString().trim());
                    jsonObject.put("loan_use", spinner1.getSelectedItem().toString().trim());
                    jsonObject.put("frequency", spinner2.getSelectedItem().toString().trim());

                    JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, "http://192.168.89.211:5000/calculate", new Response.Listener<JSONObject>() {
                        @Override
